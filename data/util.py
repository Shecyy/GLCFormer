...
def calc_light_color(x):
    """
    估计图像x的亮度和颜色
    x: tensor, CHW, RGB, 范围:[0, 1]
    """

    x = x.permute(1, 2, 0).numpy() * 255.0

    # 多尺度平滑
    kernel_size = 3
    G1 = cv2.blur(x, (kernel_size, kernel_size)) * 1.0 / 255.0
    kernel_size = 5
    G2 = cv2.blur(x, (kernel_size, kernel_size)) * 1.0 / 255.0
    kernel_size = 7
    G3 = cv2.blur(x, (kernel_size, kernel_size)) * 1.0 / 255.0

    # 亮度、颜色估计
    temp = (G1 * G2 * G3) ** (1 / 3)
    light = temp[:, :, 0:1] * 0.299 + temp[:, :, 1:2] * 0.587 + temp[:, :, 2:3] * 0.114
    color = x / 255.0 / (light + 0.000001)
    light = torch.Tensor(light).float().permute(2, 0, 1)
    color = torch.Tensor(color).float().permute(2, 0, 1)
    return light, color
...
def calc_light_light_diff(x):
    ...
    # 亮度估计、亮度差估计
    light = temp[0:1] * 0.299 + temp[1:2] * 0.587 + temp[2:3] * 0.114
    light_diff = np.abs(light - np.mean(light))
    light_diff = light_diff / (np.max(light) + 0.00000001)  # 新加的Norm

    light = torch.Tensor(light).float()
    light_diff = torch.Tensor(light_diff).float()
    light_diff = torch.clamp(light_diff, min=0, max=1.0)
    return light, light_diff
