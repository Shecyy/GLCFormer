...
class VideoSameSizeDataset(data.Dataset):
    def __init__(self, opt):
        ...
        for subfolder_LQ, subfolder_GT in zip(subfolders_LQ, subfolders_GT):
            ...
            self.data_info['idx'].append('{}/{}'.format(count, len(subfolders_LQ)))  # 源码是subfolder_LQ，并且没有下行的+1
            count = count + 1
            if self.cache_data:
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            img_LQ = img_LQ[0]
            img_GT = img_GT[0]
        light_LQ, light_diff_LQ = util.calc_light_light_diff(img_LQ)
        return {
            'LQs': img_LQ,
            'GT': img_GT,
            'prior': {'light_LQ': light_LQ, 'light_diff_LQ': light_diff_LQ},
            'folder': folder,
